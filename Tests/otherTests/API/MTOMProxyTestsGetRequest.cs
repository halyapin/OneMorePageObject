...
{
    class MTOMProxyTestsGetRequest : TestBase
    {
        [Test, Category("API")]
        public void TestGetRequestXmlProxyMtom()
        {
            ...
            else Assert.IsFalse(getResponseNode.HasChildNodes, "При запросе GetRequest поставщиком шлюз вернул пустой ответ. Для корректной работы теста необходимо убедиться, что на шлюзе есть неподтвержденные неэкспортированные запросы для поставщика.");
        }
    }
}
